...
#| label: advanced-oo-comparison
#| echo: true

# Build comparison data from the two tables prepared earlier
comparison_data = (
    avgDF_2010[['home', 'totalRuns']].rename(columns={'totalRuns': 'runs_2010'})
    .merge(
        avgDF_2021[['home', 'totalRuns']].rename(columns={'totalRuns': 'runs_2021'}),
...
# Fix a single stadium order (sorted by 2010) so the two panels line up
order = (comparison_data
...
# Common x-limit across both panels for fair comparison
xmax = 1.10 * max(comp_ord['runs_2010'].max(), comp_ord['runs_2021'].max())

# Create the two-panel figure (Object-Oriented Matplotlib)
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 7), sharey=True)

# Left panel: 2010
ax1.barh(comp_ord['home'], comp_ord['runs_2010'], color=colors_2010)
ax1.set_title("Average Runs per Game — 2010", pad=12, fontweight="bold")
...
ax2.set_title("Average Runs per Game — 2021", pad=12, fontweight="bold")
...
if "COL" in comp_ord['home'].values:
    yidx = comp_ord.index[comp_ord['home'] == "COL"][0]

    # 2010 annotation
    x2010 = comp_ord.loc[yidx, 'runs_2010']
    ax1.annotate(f"COL: {x2010:.2f}",
                 xy=(x2010, yidx),
                 xytext=(x2010 + 0.4, yidx),
                 va="center",
    ...
    # 2021 annotation
    x2021 = comp_ord.loc[yidx, 'runs_2021']
    ax2.annotate(f"COL: {x2021:.2f}",
                 xy=(x2021, yidx),
                 xytext=(x2021 + 0.4, yidx),
                 va="center",
                 bbox=dict(facecolor="white", alpha=0.8, edgecolor="none"),
                 arrowprops=dict(arrowstyle="->", color="darkorchid", lw=1.5),
                 color="darkorchid", fontweight="bold", fontsize=9)

plt.suptitle("Run-Friendliness by Stadium: 2010 vs 2021 (Colorado Highlighted)",
